...
def cbh_store2energy(cbh_store, levels_of_theory=['B3LYP', 'HF', 'MP2', 'CCSD(T)'], key_fn="fragment_lookup/keys.txt", energy_fn="fragment_lookup/lookup_energy.txt"):
          ...
          if len(val.keys()) <= 1:
            ...
            for mol, coeff in val.items():
              if mol not in cbh_lookup:
                ...
                e += sign * float(coeff) * float(cbh_lookup[mol][0][idx])
